...
param_list: param_list COMMA type ID
          ...
          | param_list COMMA type ID LBRACK RBRACK
          {
              add_param(&$$, $3); // Array parameters treated as base type
              param_count++;
          }
          | type ID
          {
          ...
          {
              $$ = NULL;
              add_param(&$$, $1); // Array parameters treated as base type
              param_count = 1;
          }
          | /* empty */
          {
...
exp: constant
   ...
   | STRING
   {
       $$ = CHAR_TYPE; // String literals are char type
   }
   | ID 
   {
       if(idcheck($1))
